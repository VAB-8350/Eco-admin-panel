...
import { Search, Plus, Pencil, UserRoundX, UserRound, Building } from 'lucide-react'
  ...
  const columns = [
    {
      ...
      cell: ({ row: { original } }) => (
        original.customer_type === 'INDIVIDUAL'
          ? <div className='flex gap-1 items-center'><UserRound className='w-5 h-5' /> Individual</div>
          : <div className='flex gap-1 items-center'><Building className='w-5 h-5' /> Empresa</div>
      )
    },
    {
      header: 'Nombre',
